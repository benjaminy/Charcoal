...
function atomicify( f )
{
    ...
    async function wrapper( ...params )
    {
        ...
        let generator_or_promise = f( ...params );
        /* TODO: handle atomic functions */
        if( generator_or_promise == null )
        {
            throw new Error( "atomicify passed (" + typeof( f )
                             + "). Needs generator function or atomic function." );
        }
        let is_generator = typeof generator_or_promise[Symbol.iterator] === "function";
        actx[ CALL_STACK_TAG ].push( generator_or_promise );

        function exitCall()
        {
            assert( actx[ CALL_STACK_TAG ].length > 0 );
            let g = actx[ CALL_STACK_TAG ].pop();
            assert( g === generator_or_promise );
        }
        ...
        try {
            ...
            if( is_generator )
            {
                while( true ) /* return in loop, when generator is done */
                {
                    ...
                    if( is_err )
                        var generated = generator_or_promise.throw( val );
                    else
                        var generated = generator_or_promise.next( val );
            ...
            {
                await notBlocked();
                return await generator_or_promise;
            }
        }
        catch( err ) {
            exitCall();
...
function makeContext( child, parent )
{
    ...
    child[ SCHEDULER_TAG ]  = parent ? parent[ SCHEDULER_TAG ] : null;
...
return function activate( ...params_plus_f )
{
    ...
    assert( isContext( parent ) );
    let sched = parent[ SCHEDULER_TAG ];
    ...
    let child = new Promise( function( resolve, reject ) {
        ...
        if( sched.atomic_stack.length > 0 )
        {
            /* XXX More cases */
            let atomic_stack = sched.atomic_stack;
...
return async function atomic( ...params_plus_fn )
{
    ...
    assert( isContext( actx ) );
    let scheduler = actx[ SCHEDULER_TAG ];
    assert( scheduler.active_actxs.has( actx[ ID_TAG ] ) );
