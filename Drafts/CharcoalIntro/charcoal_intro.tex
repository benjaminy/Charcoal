...
This family is defined in contrast to preemptive abstractions and abstractions that emphasize isolation and/or immutability.
...
We found that call frame allocation is critical, and we introduce a new call frame allocation strategy called \emph{hot stacking}, which provides low memory and time overhead.
Hot stacking is enabled by activities in the sense that we believe implementing it in the context of other thread-like primitives would be substantially more complex and potentially have bad uncommon case performance characteristics.
...
Reasoning (both human and automated) about calls is much easier if they are atomic.
...
If a programmer assumes that a call is atomic (for example by assuming that the value stored in a global variable is constant before and after a call) but the call is in fact interruptible, the result can be an atomicity violation.
...
This is very bad for maintenance...
