...
\title{To Async or Not to Async; Better Cooperative Threads are the Answer}

\author{Benjamin Ylvisaker} {Colorado College, [optional: Address], Country \and \url{http://www.myhomepage.edu} } {bylvisaker@coloradocollege.edu} {https://orcid.org/0000-0002-1825-0097} {(Optional) author-specific funding acknowledgements}
...
In this paper we analyze some of the software engineering problems related to coroutines and advocate for cooperative threads as a better alternative.
...
We present evidence that, though not a crisis, this problem does come up in practice.

Existing implementations of cooperative threads have weaknesses relative to coroutines.
Two important ones are not knowing or being able to control the atomicity of procedure calls at call sites, and the efficiency of call frame allocation.
...
Intra-process multitasking is a feature which has been implemented using many programming language and system software primitives over the years (e.g. event loops/callbacks, coroutines, multithreading).
...
We briefly note that pressure to make parallel programs (i.e. those that run on multiple processors) has also increased, but that is \emph{not} the subject of this paper or the project it describes.

In many programming communities preemptive multithreading is considered too error prone to be used for basic multitasking needs.
These communities use some flavor of cooperative concurrency.
In recent years there has been a substantial increase in the popularity of a flavor of coroutines, often called \emph{async functions{\slash}methods{\slash}procedures}.
This family of abstractions allows programmers to write code that looks and behaves mostly like conventional procedural code.
However, in the context cooperative concurrency frameworks, ``normal'' procedures are \emph{atomic}, whereas async procedures are \emph{interruptible}.
This distinction is essential when a procedure call might take a long time to complete, or even pause indefinitely.
Using an atomic procedure call in such cases can easily lead to starvation{\slash}unresponsiveness, whereas using an interruptible procedure call makes it possible for concurrent tasks to do something.

While async procedures are certainly a major improvement over event loop/callback code from a usability perspective, they still suffer from an important software engineering challenge.
For some procedures, essentially all calls are either sure to be fast, in which case it is better for those calls to be atomic; for some procedures 
...
% [\texttt{github.com/benjaminy/Charcoal}].
...
%% There is a dynamically scoped \emph{atomic} primitive that allows programmers to enforce atomicity for arbitrary blocks by overriding/suppressing yields.
%% From a programmer's perspective the atomic primitive is similar to \emph{atomic} in transactional memory abstractions.
...
%% \emph{Threads} are a well-known technology that can be used for writing multitasking software\footnotemark{}.
...
%% It is still too hard in the opinion of many software architects.
...
%% Of course there are concurrency frameworks that are even further afield.
%% For example, there are many models that focus on isolation and communication/message passing.
%% We discuss connections with these models briefly at the end of the paper.
%% The main body of this paper is focused on comparisons to shared-memory cooperative concurrency models.





\section{The Coroutine Menagerie}

Coroutine-like abstractions come in a number of different flavors, which makes them easy to miscommunicate about.
de Moura and Ierusalimschy\cite{MouraXXX} give a nice survey of some of the interpretations.
Felleisen and Hieb \cite{HXXX} give a formal semantics in terms of continuations.
Some languages (e.g. C#) further muddy the waters by hybridizing coroutines and multithreading.
The essential feature of coroutine-like abstractions (herafter, just \emph{coroutines}) is that they can use a \emph{yield} primitive to suspend their own execution and resume the execution of some other suspended coroutine.

There are two families of use patterns for coroutines: generators and interruptible subroutines.
``Calling'' a coroutine is quite different from a regular subroutine call.
Calling a coroutine spawns a concurrent task and returns a handle for that task.
The generator pattern involves passing a sequence of input values to the coroutine via its handle, and receiving a sequence of output values back.
The interruptible subroutine pattern involves passing some parameters to a coroutine and then waiting for it to finish.
Unlike a regular subroutine call, during the execution of the coroutine, control may transfer to some other concurrent coroutine (thus ``interruptible'').

Some languages have distinct primitives for these two patterns.
For example, JavaScript has both \texttt{function*} (generators) and \texttt{async function} (interruptible subroutines).
The difference between the two is superficial.
The standard for async functions gives a definition by translation to generators\cite{XXX}.
Defining generators in terms of async functions is also straightforward: the caller passes an extra parameter that is used as the handle to pass values back and forth, generator-style.

It seems that in practice interruptible subroutine is a more commonly used pattern than generator.
[evidence]
...
\section{Interruptable Procedure Call is the Most Common Pattern}

We begin not with the most serious problem with coroutines, but one that sets up later discussion well.
Coroutines can be used in a few different patterns.
Some of the common ones include interruptable procedure calls, producer-consumer pipeline, concurrent tasks.
In practice it seems that interruptible procedure calls are by far the most common use.
In XXX code base the yield/await keyword is
...
\section{Coroutine Problem 1: Routine Author Chooses Atomic vs Interruptible}

\section{Coroutine Problem 2:}
...
\footnotetext{It does not matter whether this is because \texttt{otherProcedure} is computationally intensive or because it waits (blocks) for some event outside the program itself.}
...
\footnotetext{There are many different terms used and subtle semantic differences in the coroutine/async procedure space.
We will mostly ignore these subtle differences and use \emph{coroutine} generically.}
...
%% With the addition of atomic, activities can be seen as quite similar to coroutines with a complementary default.
%% Normal procedures in an activity framework are like coroutines and normal procedures in a coroutine framework are like atomic procedures in an activity framework.
%% As similar as this argument makes coroutines and activities seem, we believe the differences are still significant.
%% For example, in an activities framework, all procedures are part of the same type; normal procedures and atomic procedures are not distinguished at the type level.
