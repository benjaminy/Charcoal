...
[tl;dr]
<p>
Existing frameworks for writing multitasking software (event dispatchers,
...
<p>
Under the hood activities look just like cooperative threads, but the language definition includes frequent implicit <em>yields</em>, which make activities feel more preemptive (thus <em>pseudo-preemptive</em>).
A simple synchronization primitive called <em>no-yield</em> makes it relatively easy to avoid many concurrency bugs, the bane of multithreaded software.
</p>
</div>
...
<td class="color_good"><span class="mono">no_yield</span>:
...
<td class="color_pretty_good">Avoid wrapping long-running blocks
in <span class="mono">no_yield</span> and you're golden</td>
</tr>
<tr>
...
<p>
There are at least three families of concurrency/multitasking
...
state of multitasking software in a way that can integrate more easily
...
an attempt to do both parallelism and multitasking; I think those should
really be handled by two different frameworks.
</p>

<p>
[Edit: the C++ community is moving towards including TM in the language standard.
This is a topic worthy of a longer discussion, but I believe that the flavor that they're working on will not take over the world any time soon.]
</p>
...
<p>
Charcoal code runs in one of two modes: no-yield or might-yield.
In no-yield mode, it runs as quickly/<wbr>efficiently as equivalent C code.
However, in might-yield mode things slow down quite a bit.
It's not conceptually hard to tune CPU-bound code to be in no-yield mode most of the time, but it does take some effort.
This is never catastrophic, it just means that untuned CPU-bound code is somewhat less efficient than it could be.
</p>
...
software.  As early as the 1960s, multitasking software was being
developed (for
example, <a href="http://en.wikipedia.org/wiki/Sketchpad">Ivan
Sutherland's Sketchpad</a>).  Multitasking software has a dramatically
different interface with its environment.  Multitasking software can:
<ul>
...
<p>Multitasking software is generally considered harder to write and
...
multitasking carefully sequestered in as small a corner of the code as
possible.  Much to the chagrin of the programmers who prefer thinking
procedurally, the relative importance of multitasking in mainstream
...
<p>The two dominant abstractions for multitasking software are event
...
My one sentence summary of these arguments: Event handlers are great for relatively simple multitasking patterns, but scale very badly as program logic becomes more complex (<a href="http://callbackhell.com/">"callback hell"</a>).
Threads allow complex patterns to be programmed in a natural (procedural) style, but make it extremely hard to avoid nasty concurrency bugs (data races, deadlocks, and atomicity violations, oh my).</p>
...
<p>
<strong>An aside about parallelism</strong>.  A major difference between event handlers and threads is that threads can run in parallel on separate processors, whereas event handlers cannot.
While this difference is hugely significant <em>in general</em>, from the <em>specific</em> perspective of <em>multitasking</em> it is almost entirely irrelevant.
Multitasking and parallelism are completely different topics, so the fact that threads can be run in parallel is beside the point when we are talking about how to write multitasking software.
</p>

<p>
<strong>The main idea(finally!)</strong>:
If event handlers and threads both have such serious problems, why are they used so heavily in modern software?
Quite simply, it's because nothing better exists yet.
This project introduces a new primitive (called an activity) that combines the advantages and mitigates the disadvantages of event handlers and threads for programming multitasking software.
</p>

<p>
On the way to explaining what activities are, we are going to take a brief detour through cooperative threads.
Cooperative threads are like conventional (a.k.a. preemptive) threads, except only one is allowed to run at a time and the active cooperative thread must explicitly invoke a <em>yield</em> primitive to give the system the option of switching to a peer cooperative thread.
The nicest feature of cooperative threads is that between yield invocations threads cannot interrupt each other, which makes avoiding (some) concurrency bugs easier.
</p>

<p>
Cooperative threads are a kind of compromise between event handlers and threads, and they do combine some of the advantages of both.
Unfortunately, cooperative threads come with their own nasty drawback: application programmers are responsible for getting yield invocations in just the right places.
If a cooperative thread doesn't yield frequently enough it can starve other threads of processor time and make the whole application unresponsive.
...
<p>
The "<a href="http://en.wikipedia.org/wiki/Goldilocks_principle">Goldilocks</a>" yield frequency is especially hard to maintain when a program is composed of multiple libraries and frameworks (as most modern applications are).
What assumptions did the authors of the various components make about whether code in other components will yield or not?
Answering this question is important and extremely hard in general.
This problem has limited the adoption of cooperative threading to the margins of the software industry.
However, in the name of making multitasking programming easier (especially for applications that run across the internet), cooperative threads (and their cousins, coroutines and generators) have started showing up in some mainstream software (<a href="http://code.activestate.com/recipes/466008-simple-cooperative-multitasking-using-generators/">Python</a>, <a href="http://lua-users.org/wiki/MultiTasking">Lua</a>, <a href="http://ruby-doc.org/core-2.0/Fiber.html">Ruby</a>, <a href="http://msdn.microsoft.com/en-us/library/hh191443.aspx">.NET</a>, <a href="https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Iterators_and_Generators">Javascript</a>, etc.)
</p>
...
<p>
These frequent yields could easily reintroduce the concurrency bugs that bedevil multithreaded software.
The primary tool to prevent this is the ability to declare any procedure or block of code "no-yield".
Within the dynamic scope of a no-yield block the current activity cannot be interrupted.
</p>
...
<p>
I believe activities inhabit an interesting new part of the universe of multitasking software primitives.
In particular I aim to show that multi-activity software can be:
</p>

<ul>
<li><em>Simple</em>.  Like multithreaded software, multitasking can be written in a natural, procedural style.  (i.e. no callback hell.)</li>
<li><em>Reliable</em>.  The careful balancing of frequent implicit yields and simple synchronization make it easy (or at least easier) to avoid the most problematic concurrency bugs.</li>
<li><em>Efficient</em>.  Activities have been very carefully designed to allow implementations that offer super-efficient multitasking primitives (activity creation, context switching, synchronization, activity memory overhead, &hellip;) while avoiding slowing down sequential code.</li>
</ul>
...
software (including multitasking software) can be made doable by a strong
...
memory is <em>the</em> solution to the multitasking software challenge.
